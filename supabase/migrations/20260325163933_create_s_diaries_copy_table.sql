/*
  # Create s_diaries_copy table (copy of s_diaries)
  ...
  2. Security
    - Enable RLS on `s_diaries_copy` table
  ...
  3. Important Notes
    - This is a copy of the s_diaries table structure
...
-- Create s_diaries_copy table
CREATE TABLE IF NOT EXISTS s_diaries_copy (
...
-- Enable RLS
ALTER TABLE s_diaries_copy ENABLE ROW LEVEL SECURITY;
...
CREATE INDEX IF NOT EXISTS idx_s_diaries_copy_user_date ON s_diaries_copy(user_id, date);
CREATE INDEX IF NOT EXISTS idx_s_diaries_copy_date ON s_diaries_copy(date);
